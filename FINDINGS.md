...
`UpdateTimer` refuses to rotate within **60 minutes** of the last time the wallpaper was touched (`WallpaperRefresh`, with `Rotation` alongside it, under `HKCU\…\DesktopSpotlight`). The constant is the component's own; the elapsed values are minutes. And a refused run writes **nothing** — `UpdateTimer`, `LastBackgroundTaskRunDate` and the rest are stamped only on success. From the outside, a gated run and a task that never started are indistinguishable.
